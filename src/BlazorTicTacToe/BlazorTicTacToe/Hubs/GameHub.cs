...
namespace BlazorTicTacToe.Hubs
{
    public class GameHub : Hub
    {
        ...
        {
            Console.WriteLine($"Player with Id '{Context.ConnectionId}' conndected.");

            await Clients.Caller.SendAsync("Rooms", _rooms.OrderBy(r => r.RoomName));
        }

        public async Task<GameRoom> CreateRoom(string nameRoom, string playerName)
        {
            var roomId = Guid.NewGuid().ToString();
            var room = new GameRoom(roomId, nameRoom);
            _rooms.Add(room);
